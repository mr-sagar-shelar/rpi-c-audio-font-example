...
void play_melody(const char *device) {
    ...
    // Use ALSA's "plug" interface automatically if needed by appending "plug"
    // But we'll trust the user's selection (if they pick plughw: it handles conversion)
...
void change_device(char *current_device) {
    ...
    while (*n != NULL && i < 50) {
        ...
        if (name != NULL) {
            // Filter out purely virtual devices to keep the list clean
            if (strncmp(name, "hw:", 3) == 0 || strncmp(name, "plughw:", 7) == 0 || 
                strcmp(name, "default") == 0 || strncmp(name, "sysdefault:", 11) == 0) {
                
                // Clean up newlines in the ALSA descriptions
                if (desc) {
    ...
    if (choice > 0 && choice < i) {
        ...
        current_device[63] = '\0'; // Ensure null termination
